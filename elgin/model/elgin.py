...
---------------------
S2  Evaporation tracking — d_p0 stored at injection; d_p(t) updated
    each step via Wells' d^2-law; current d_p passed to LagrangianGNN.

S7  Two-way Eulerian-Lagrangian coupling — when cfg.use_two_way_coupling
    and local particle volume fraction exceeds cfg.two_way_vol_frac_thr,
    a momentum source term S_i = -sum_{p in cell i} F_drag^(p) / V_i
    is computed and added to the fluid field as an explicit correction to
    the EulerianGNN output before the pressure projection.

    This mirrors the two-way coupling in OpenFOAM's
    ReactingMultiphaseParcelFoam and the plasma PIC-GNN of Mlinarević
    et al. (2025).

    The coupling is OFF by default (one-way assumption is valid for the
    dental aerosol problem where phi_v < 0.001 %) and is activated via
    cfg.use_two_way_coupling = True.
"""
...
class CfdGNN(nn.Module):
    ...
    S2: Accepts d_p0 (initial diameter) and updates d_p(t) each step
        using Wells' evaporation model.

    S7: Optionally computes two-way momentum coupling when
        cfg.use_two_way_coupling = True.
    """
    ...
    def step(
        ...
        # When True the EulerianGNN and pressure-projection steps are skipped
        # and ``fluid_field`` is used directly for the fluid→particle IDW
        # interpolation.  This replicates *exactly* what rollout.py --freeze_fluid
        # does at inference: it never calls model.step() at all — it interpolates
        # the raw GT fluid to particles and calls lagrangian_gnn.next_position()
        # directly.  Use bypass_eulerian=True in any training stage whose
        # rollout counterpart uses --freeze_fluid (Stage 2 / Stage 4 BPTT when
        # use_gt_fluid_in_bptt=True) so the Lagrangian sees the same fluid
        ...
        if bypass_eulerian:
            # Skip EulerianGNN + pressure projection — use input fluid directly
            # for particle IDW.  This exactly matches rollout.py --freeze_fluid
            # which never calls model.step() and interpolates raw GT fluid to
            # particles directly.  Keeping new_fluid = fluid_field means the
            # returned "fluid_field" is the GT input, so mse_fluid in the loss
            # measures the one-step GT change (near zero for steady RANS) rather
            # than the Eulerian's garbage prediction.
        ...
        # Wall reflection: when a particle overshoots a boundary by δ, place it
        # at (wall - δ) so the implied velocity in the next step reverses sign.
        # This avoids the "ceiling pile-up" that occurs with simple clamping:
        # clamped particles have constant history at the wall, so the LSTM
        # keeps predicting upward motion and they never escape.
        if clip_particles:
