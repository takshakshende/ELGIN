# =============================================================================
#  run_case03.ps1
#  Single-case CFD-GNN pipeline (Sweep_Case_03 only, 1000 particles).
#
#  Pipeline: extract -> mesh -> train -> rollout -> animate.
#  Output layout (mirrors run_full_training.ps1):
#
#    experiments\cfd_gnn_case03\
...
#        animations\      case_03.mp4
#        metrics\         case_03_metrics.json
#
#  Usage:
#    cd $PSScriptRoot\..
#    .\run_case03.ps1                       # full pipeline, 100 epochs
#    .\run_case03.ps1 -Epochs 200           # longer training
#    .\run_case03.ps1 -SkipExtract          # reuse existing case_single.npz
#    .\run_case03.ps1 -SkipExtract -SkipMesh -SkipTrain   # rollout + animate only
# =============================================================================

[CmdletBinding()]
param(
    # --- Epoch budget -------------------------------------------------------
    # 300 epochs is the recommended minimum for single-case training.
    # Stage split (train_single.py) is 10/25/10/55 so Stage 4 BPTT gets
    # 55 % of the budget (~165 epochs at 300 total) to fix rollout drift.
    [int]    $Epochs       = 300,
    ...
    # 3 steps = 0.3 s of rollout per BPTT unroll (at dt=0.1 s).
    # Each BPTT step is sequential (cannot be parallelised), so bptt_steps
    # is the dominant cost factor in Stage 4:
    #   bptt_steps=3  → ~8  min/epoch  (default — best speed/quality balance)
    #   bptt_steps=5  → ~13 min/epoch  (current test run)
    #   bptt_steps=10 → ~26 min/epoch  (diminishing returns)
    #   bptt_steps=30 → ~75 min/epoch  (too slow for single-GPU)
    # Note: bptt loss is near-zero even at step 1 since Stage 2 already
    # trains one-step accuracy.  The main benefit of Stage 4 is the 2×
    # noise augmentation (noise_std*2) which reduces history-encoder drift.
    [int]    $BpttSteps    = 3,

    # Weight of BPTT rollout loss vs one-step supervised loss in Stage 4.
    # 0.7 puts most pressure on fixing long-horizon drift.
    [double] $BpttWeight   = 0.7,

    # --- Stage 4 only (resume from existing Stage-3 checkpoint) -------------
    # Pass -Stage4Only to skip Stages 1-3 and jump straight to Stage 4 BPTT
    # using the best.pt already saved in the model directory.  Useful when
    # Stage 4 timed out or you want to re-run it with different bptt_steps.
    [switch] $Stage4Only,

    [string] $CaseName     = "Sweep_Case_03",
    [string] $Device       = "cuda",
    [switch] $SkipExtract,
    [switch] $SkipMesh,
    [switch] $SkipTrain,
...
$EXP_DIR    = Join-Path $EXP  "cfd_gnn_case03"
...
# =============================================================================
#  Helpers (same style as run_full_training.ps1)
# =============================================================================
...
#  Sanity checks
# =============================================================================
Write-Banner "CFD-GNN single-case run  ($CaseName, $NParticles particles)"
...
if ($SkipExtract -and (Test-Path $NPZ_PATH)) {
    ...
    $t = Invoke-Step "Extract  $CaseName" @(
        "python", "-u", "cfd_gnn\data\extract_fields.py",
        "--case_dir",    $OF_CASE,
        "--output",      $NPZ_PATH,
        "--t_start",     "$TStart",
        "--t_end",       "$TEnd",
...
if ($SkipMesh -and (Test-Path $MESH_PATH)) {
    ...
    $t = Invoke-Step "Mesh graph" @(
        "python", "-u", "cfd_gnn\data\mesh_to_graph.py",
        "--case_dir", $OF_CASE,
        "--output",   $MESH_PATH
    ) "$LOGS_DIR\mesh.log"
...
# =============================================================================
#  STEP 3 - Train CFD-GNN  (re-uses train_single.py with the new defaults)
# =============================================================================
if (-not $SkipTrain) {
    $env:PYTORCH_CUDA_ALLOC_CONF = "expandable_segments:True"

    $trainCmd = @(
        "python", "-u", "cfd_gnn\train_single.py",
        "--case_dir",    $OF_CASE,
        "--out_dir",     $DATA_DIR,
        "--model_dir",   $MODEL_DIR,
        "--device",      $Device,
        ...
        #   the --freeze_fluid flag used at rollout time.  Without this, Stage 4
        #   trains against a drifting Eulerian the model never sees at inference.
        "--freeze_fluid_stage4",
        "--bptt_steps",          "$BpttSteps",
        "--bptt_weight",         "$BpttWeight",
        "--bptt_rollout_noise",  "0.01",
        # ── Stage 3 PDE lambda weights ───────────────────────────────────────
        # Reduced from defaults (mom=0.05, cont=0.10, turb=0.02) to prevent
        # the RANS momentum residual (which is ~4 billion on a poorly-initialised
        # Eulerian) from dominating Stage 3 and erasing the Lagrangian learning
        # from Stage 2.  Smaller lambdas keep Stage 3 as a gentle regulariser
        # rather than an aggressive physics override.
        "--lambda_mom",  "0.001",
        "--lambda_cont", "0.01",
        "--lambda_turb", "0.001",
        "--skip_extract",
    ...
    # Jump directly to Stage 4 BPTT if requested, resuming from best.pt
    if ($Stage4Only) { $trainCmd += @("--start_stage", "4") }

    $t = Invoke-Step "Train CFD-GNN ($Epochs epochs)" $trainCmd "$LOGS_DIR\train.log"
...
if (-not $SkipRollout) {
    ...
    if (Test-Path $met) {
        Copy-Item $met (Join-Path $MET_DIR "case_03_metrics.json") -Force
    }
}

# =============================================================================
#  STEP 5a - Particle-only comparison animation (existing)
# =============================================================================
if (-not $SkipAnimate) {
    $rolloutNpz = Join-Path $ROLL_DIR "rollout.npz"
    $gtNpz      = Join-Path $ROLL_DIR "gt.npz"
    if ((Test-Path $rolloutNpz) -and (Test-Path $gtNpz)) {
        $t = Invoke-Step "Animate $CaseName" @(
            "python", "-u", "scripts\animate_particles.py",
            "--input",  $rolloutNpz,
            "--input2", $gtNpz,
            "--label",  "CFD-GNN ($CaseName)",
            "--label2", "OpenFOAM GT",
            "--output", (Join-Path $ANIM_DIR "case_03.mp4"),
            "--format", "mp4",
            "--fps",    "15"
        ) "$LOGS_DIR\animation.log" -AllowFail
        $summary.Add(("Animate                 : {0:hh\:mm\:ss}" -f $t))
    } else {
        Write-Host "  [skip] Cannot animate -- rollout.npz or gt.npz missing." -ForegroundColor Yellow
    }
}

# =============================================================================
#  STEP 5b - Fluid velocity + particles animation
#            Shows steady-state air flow field with animated aerosol cloud.
#            Produces two files:
#              fluid_speed_particles.mp4  — speed (|U|) colourmap + GNN particles
#              fluid_speed_compare.mp4   — same fluid bg, GNN vs GT side-by-side
# =============================================================================
if (-not $SkipAnimate) {
    $rolloutNpz = Join-Path $ROLL_DIR "rollout.npz"
    $gtNpz      = Join-Path $ROLL_DIR "gt.npz"
    if (Test-Path $rolloutNpz) {
        # Single panel: fluid speed + GNN particles
        $t = Invoke-Step "Fluid+particle animation (single)" @(
            "python", "-u", "cfd_gnn\animate_fluid_particles.py",
            "--rollout", $rolloutNpz,
        ...
        # Two-panel: fluid speed + GNN vs GT side-by-side
        if (Test-Path $gtNpz) {
        ...
        Write-Host "  [skip] No rollout.npz found for fluid animation." -ForegroundColor Yellow
    }
}
...
Write-Host "    Animation   : $(Join-Path $ANIM_DIR 'case_03.mp4')"
Write-Host ""
